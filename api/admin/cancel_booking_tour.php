<?php
    ...
    $sql_check = "SELECT * FROM `booking_order_tour` WHERE `booking_id` = ?";
    ...
    if (isset($data['cancel_booking'])) {
        $query = "UPDATE `booking_order_tour` SET `booking_status`= ?, `refund`=? WHERE `booking_id` = ?";
        $values = ['cancelled', 0, $booking_id];
        $res = $p->execute_query($query, $values, 'sii');

        if ($res) {
            echo json_encode(['status' => 'success', 'message' => 'Đơn đặt tour đã được hủy']);
        } else {
            echo json_encode(['status' => 'error', 'message' => 'Hủy đơn đặt tour không thành công.']);
        }
        exit();
    }
?>