...
if (empty($username) || empty($password) || empty($email) || empty($phoneNumber) || empty($address) || empty($role)) {
...
if ($check && count($check) > 0) {
    ...
    $sql = "INSERT INTO `employees` (username, password, email, phoneNumber, address, role, permissions) VALUES (?, ?, ?, ?, ?, ?, ?)";
    $params = [$username, $hashedPassword, $email, $phoneNumber, $address, $role, $permissions];
