...
$sql_check_tour = "SELECT id FROM tour1 WHERE id = ?";
...
$sql_insert = "INSERT INTO `lichtrinhtour`(`date`, `image`, `schedule`, `locations`, `id_tour`) VALUES (?, ?, ?, ?, ?)";
$values = [$date, $iconPath, $schedule, $locations, $id_tour];
$result = $p->execute_query($sql_insert, $values);

if ($result) {
    $imageUrl = "http://localhost/api-dulich-main/api-dulich/api/Images/lichtrinhtour/" . $iconPath;
    ...
    echo json_encode(['status' => 'error', 'message' => 'Thêm lịch trình tour không thành công.']);
}
