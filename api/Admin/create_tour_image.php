...
if ($result_check_tour->num_rows === 0) {
    echo json_encode(['status' => 'error', 'message' => 'tour_id không tồn tại.']);
    exit();
}
...
// Tải lên biểu tượng
$iconPath = uploadImage($_FILES['image'], 'tourimage');
...
$sql_insert = "INSERT INTO `tours_images`(`tour_id`, `image`) VALUES (?, ?)";
$values = [$tour_id, $iconPath];
$result = $p->execute_query($sql_insert, $values);

if ($result) {
    $imageUrl = "http://localhost/api-dulich-main/api-dulich/api/Images/tourimage/" . $iconPath;
    http_response_code(201); // Đã tạo
    echo json_encode(['status' => 'success', 'message' => 'Hình ảnh tour đã được thêm thành công.', 'image_url' => $imageUrl]);
} else {
    http_response_code(500); // Lỗi máy chủ nội bộ
    echo json_encode(['status' => 'error', 'message' => 'Thêm hình ảnh tour không thành công.']);
}